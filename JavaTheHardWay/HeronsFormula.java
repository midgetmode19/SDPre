public class HeronsFormula {
  ...
  //This function computes the area of a trianle with side lengths a, b, ca
  public static double triangleArea(int a, int b, int c) {
    double s, A;
    s = (a+b+c) / 2.0;
    A = Math.sqrt(s*(s-a)*(s-b)*(s-c));
    return A; //must return value of function computation
  }
  //Code with functons is much shorter than code without functions.
  //Fixing the logical error for division by 2 issue would be much more
  //annoying without functions, because it would have to be changed
  //every place the formula is written.
}
